...
bool ble_srv_temp_sensor_init(void)
{
    if (s_temp_sensor_initialized) {
        BLE_SRV_LOGW(TAG, "Temperature sensor already initialized");
        return true;
    }
    ...
    if (ret != ESP_OK) {
        BLE_SRV_LOGE(TAG, "Failed to install temperature sensor: %s", esp_err_to_name(ret));
        return false;
    }

    ret = temperature_sensor_enable(s_temp_sensor);
    if (ret != ESP_OK) {
        BLE_SRV_LOGE(TAG, "Failed to enable temperature sensor: %s", esp_err_to_name(ret));
    ...
    BLE_SRV_LOGI(TAG, "Temperature sensor initialized");
    return true;
}
...
bool ble_srv_temp_sensor_read(float *out_celsius)
{
    if (!s_temp_sensor_initialized || s_temp_sensor == NULL) {
        BLE_SRV_LOGE(TAG, "Temperature sensor not initialized");
        return false;
    }

    esp_err_t ret = temperature_sensor_get_celsius(s_temp_sensor, out_celsius);
    if (ret != ESP_OK) {
        BLE_SRV_LOGE(TAG, "Failed to read temperature: %s", esp_err_to_name(ret));
        return false;
    }
...
void ble_srv_temp_sensor_deinit(void)
{
    ...
    BLE_SRV_LOGI(TAG, "Temperature sensor deinitialized");
}
...
bool ble_srv_temp_sensor_init(void)
{
    BLE_SRV_LOGW(TAG, "Temperature sensor not supported on this chip");
    return false;
}
